...
async def clean_video(update, context, file_id, filename):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            tg_file = await context.bot.get_file(file_id, read_timeout=60, write_timeout=60, connect_timeout=60)
            await tg_file.download_to_drive(input_path)

            # Проверяем что файл скачался
            if not os.path.exists(input_path) or os.path.getsize(input_path) == 0:
                await status_msg.edit_text("❌ Не удалось скачать файл. Попробуй отправить как файл через 📎 → Файл")
                return
            ...
            if result.returncode != 0:
                # Пробуем с перекодированием если copy не сработал
                result2 = subprocess.run(
                    ['ffmpeg', '-i', input_path, '-map_metadata', '-1', '-c:v', 'libx264', '-c:a', 'aac', '-y', output_path],
                    capture_output=True, text=True, timeout=300
                )
                if result2.returncode != 0:
                    logger.error(f"FFmpeg error: {result2.stderr}")
            ...
            await status_msg.edit_text(
                "❌ Произошла ошибка.\n\n"
                "Попробуй отправить видео как файл: 📎 → Файл"
            )
